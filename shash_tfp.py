...
class Shash(tfp.distributions.SinhArcsinh):
    ...
    * The mean, skew, stddev, and variance defined by Shash MAY NOT
    be used in the loss function because the computations require computation
    of the Bessel Function of the second kind.  The TensorFlow implementation,
    ...
    def _convert_tfp_to_jones_and_pewsey(self):
        ...
        * This conversion of the parameters is neccesary to use the detailed
        ...
        if self.parameters["skewness"] is not None:
            epsilon = self.parameters["skewness"]
        else:
            epsilon = tf.zeros_like(self.parameters["loc"])

        if self.parameters["tailweight"] is not None:
            eta = (
    ...
    def median(self):
        ...
        -------
        x : Tensor of same dtype and shape as loc specified at initialization.
            The computed distribution median values.
    ...
    def mode(self):
        ...
        * We initialize the Newton's method iterations using the medians.
        ...
        y = tf.math.sinh(epsilon / delta)
        for iteration in range(MAX_ITERATION):
            S = tf.math.sinh(delta * tf.math.asinh(y) - epsilon)
            C = tf.math.cosh(delta * tf.math.asinh(y) - epsilon)

            S2 = tf.square(S)
            S3 = tf.multiply(S, S2)
            C2 = tf.square(C)
            T = tf.sqrt(1.0 + tf.square(y))

            p = y + y * S2 + delta * C * S3 * T
            dp = 1.0 + S2 + tf.square(delta) * S2 * (3.0*C2 + S2) + 2.0*delta*C*S*(1.0 + S2) * y/T
            change = p/dp
            y = y - change

            if tf.reduce_max(tf.abs(change)) < TOLERANCE:
                break
